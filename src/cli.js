...
var Terminal = {
	...
	processInputBuffer: function(cmd,notLS = true) {
		this.print($('</br>'));
		if(notLS)
		{
		...
		{
			this.print($('<p>').addClass('command').text(this.config.prompt + this.buffer.substring(0,2)));
		}
	...
	runCommand: function(text,notLS = true) {
		var index = 0;
		var mine = false;
		...
		var interval = window.setInterval($.proxy(function typeCharacter() {
			if (index < text.length) {
				...
				this.processInputBuffer(null,notLS);
			}
